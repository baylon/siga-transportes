				...
				<c:otherwise>
	 			    <c:choose>
					   <c:when test="${programarRapido}">
							<input type="button" id="btnVoltar"  value="&{'views.botoes.voltar'}" onClick="javascript:location.href='${linkTo[RequisicaoController].listar}'" class="gt-btn-medium gt-btn-left" />
					   </c:when>
					   <c:otherwise>
							<input type="button" id="btnVoltar"  value="&{'views.botoes.voltar'}" onClick="javascript:location.href=''${linkTo[MissaoController].listar}''" class="gt-btn-medium gt-btn-left" />
					   </c:otherwise>	
					</c:choose>
				</c:otherwise>
			</c:choose>
