...
public class WorkFlowNotificacao implements Task {
	...
	public void execute() {
		try {
			...
			if (executa) {
				...
				Logger.getLogger(SIGATP_WORKFLOW).info("Servico de Nofitificacao do WorkFlow desligado");
			}
			Logger.getLogger(SIGATP_WORKFLOW).info("Servico de Nofitificacao do WorkFlow finalizado");
		} catch (Exception e) {
	...
	private static Boolean enviarEmail(String titulo, Set<?> objeto, String linha) throws Exception {
		...
		boolean flagEmail = Boolean.parseBoolean(Parametro.buscarConfigSistemaEmVigor(CRON_ENVIARPARALISTA));
