...
public class RequisicaoController extends TpController {
    ...
    @Path("/salvar")
    public void salvar(RequisicaoTransporte requisicaoTransporte, TipoDePassageiro[] tiposDePassageiros, 
    				   boolean checkRetorno, boolean checkSemPassageiros) throws Exception {
    ...
    protected void carregarFinalidades() throws Exception {
    	...
    	if (autorizacaoGI.ehAdministrador()) {
    		...
    		mostrarOutra = Boolean.valueOf(Parametro.buscarValorEmVigor("mostrarTipoFinalidadeOutra", getTitular(), recuperarComplexoPadrao()));
    	}
    ...
    public List<RequisicaoTransporte> podeCancelar(final Missao missaoAExcluirNaVerificacao) throws Exception {
		List<Missao> missoesFiltradas = new ArrayList<Missao>();
		List<RequisicaoTransporte> requisicoesParaCancelar = new ArrayList<RequisicaoTransporte>();

		requisicoesParaCancelar = Lists.newArrayList(Iterables.filter(missaoAExcluirNaVerificacao.getRequisicoesTransporte(), new Predicate<RequisicaoTransporte>() {
			public boolean apply(RequisicaoTransporte requisicao) {
				return requisicao.getMissoes().size() == 1;
			}
		}));
		
		for (RequisicaoTransporte req : missaoAExcluirNaVerificacao.getRequisicoesTransporte()) {
			if (req.getMissoes().size() > 1) {
				missoesFiltradas = Lists.newArrayList(Iterables.filter(req.getMissoes(), new Predicate<Missao>() {
					public boolean apply(Missao missao) {
						return !missao.equals(missaoAExcluirNaVerificacao) && missao.getEstadoMissao().equals(EstadoMissao.CANCELADA);
					}
				}));
				
				if (missoesFiltradas.size() == req.getMissoes().size() - 1) {
