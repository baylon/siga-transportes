...
<a href="${href}" <c:if test="${not empty onclick}">onclick="${onclick}"</c:if>
				  <c:if test="${not empty classe}">class="${classe}"</c:if>> 
	<img src="${imagem}" alt="${alt}" title="${tituloCompleto}"	 width="19" height="22">
</a>